...
- **桌面（≥560px）**：侧边栏常驻（无收起按钮——PWA 独特性），尺寸流式（`clamp()`）；
  - **≥1200px**：整块限宽居中（`max-width`）+ 尺寸上档（圆环 280~400px、按钮 56px），
    两侧留白由氛围光填充，不再"右侧太空"；
  - **≥1400px**：音乐播放器**右移停靠**主区域右下，填补横屏右侧空白；
  - **≥1920px 宽且 ≥1200px 高（4K/高分屏）**：再上档（圆环 ≤460px、主按钮 64px、
    顶栏按钮 44px、播放器 ≤620px），大屏不再显得按钮小气；
- **手机（<560px）**：侧边栏变为**抽屉**（☰ 打开 + 遮罩关闭），顶栏整合模式拨杆与功能按钮，
  主按钮 52px（≥44px 触摸目标）、播放器贴底适配安全区（`env(safe-area-inset-bottom)`）；
