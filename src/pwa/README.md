...
PWA 外壳是**流式全窗口布局**（不做整壳缩放），按断点适配：

- **桌面（≥560px）**：侧边栏常驻（无收起按钮——PWA 独特性），尺寸全部流式
  （`clamp()`：侧栏宽 170~250px、计时圆环 220~330px、按钮 46px 高），大屏不再稀疏；
- **手机（<560px）**：侧边栏变为**抽屉**（☰ 打开 + 遮罩关闭），顶栏整合模式拨杆与功能按钮，
  主按钮加大到 52px（≥44px 触摸目标）、播放器贴底适配安全区（`env(safe-area-inset-bottom)`）；
