...
describe("GardenMain.vue", () => {
  ...
  it("应渲染三个导航按钮", () => {
    ...
    expect(navBtns).toHaveLength(3);
    // 签到 / 商店 / 成就墙
    expect(navBtns[0].attributes("title")).toBe("每日签到");
    expect(navBtns[1].attributes("title")).toBe("商店");
    expect(navBtns[2].attributes("title")).toBe("成就墙");
  });
  ...
  it("点击签到按钮应显示 GardenSignin 弹窗", async () => {
    ...
    // 点击签到按钮
    const signinBtn = wrapper.findAll(".garden-nav-btn")[0];
  ...
  it("点击商店按钮应显示 GardenShop 弹窗", async () => {
    ...
    const shopBtn = wrapper.findAll(".garden-nav-btn")[1];
  ...
  it("点击成就墙按钮应显示 GardenAchievement 弹窗", async () => {
    ...
    const achBtn = wrapper.findAll(".garden-nav-btn")[2];
