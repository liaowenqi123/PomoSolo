...
**及时推送纪律（强制，2026-08-16 起）：**
- **每次工作会话结束（完成一组改动 / 一个 commit 后）立即推送双仓库**，不要攒多个 commit 一起推，
  更不要"只 commit 不 push"过夜——服务器部门部署依赖 `self` 仓库、GitHub CI/Release 依赖 `origin`，
  任何一边滞后都会让线上与仓库不一致（历史教训：凌晨高频小步提交却不及时推，导致发版阻塞）；
- 推送失败 / 暂时没网时，在 commit message 或 PR 里注明"self 待同步"，网络恢复后**第一时间补推**；
- 推送前先 `git status` 检查，确认没有漏掉新文件（`??` 未跟踪）或误改了生成物（如
  `music-manifest.json` 的时间戳噪声）；只推想推的东西。
...
2. 文档同步了吗？（改代码必更文档）
3. 两个仓库都推了吗？（**每次工作完立即 push origin + self**，不攒 commit 过夜）
4. 发版前红线核验了吗？（更新链路改动必用真实发布物验签，见 §16.1）
