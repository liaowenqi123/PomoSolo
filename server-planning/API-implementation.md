...
> 部门：PWA部门 ｜ 留言类型：待配合（查证 1 项 + 确认 2 项 + 联调 1 项）

**症状（PWA v0.4.0 实测，请优先看）**：
- 电脑浏览器：点击 P2P 打洞测试时偶发显示"WebSocket 断开"（StudyRoom 弹"网络连接断开，正在自动重连…"）；
- 手机：**连流量时偶尔**断开；**连 WiFi 时现在持续断开**，重连后仍连不上；
- 已确认**不是**同账号双端登录（互踢场景排除）；"刚才还能连上、现在连不上"与使用时长相关。
- 客户端初步判断：WS 掉线后自动重连用的是已过期 access token（15 分钟）→ 每次重连都被
  服务器鉴权拒绝 → 永久连不上（0.4.1 已加"重连前自动刷新 token"+ 关闭码日志，待发布验证）。

**请服务器部门**：

1. **查证（优先）**：查看 WS 日志中该手机客户端的**断开原因与关闭码**——是服务器主动关闭
   （哪个分支/原因）还是网络层断开？token 过期后服务器是否会主动关闭已建立连接？
2. **踢人关闭码**：若存在同账号新连接踢旧连接，请用**关闭码 4001（logged-in-elsewhere）**；
   PWA 已实现"仅 4001 视为被踢并停止自动重连"，其余关闭码一律自动重连（避免误停）。
3. **鉴权失败关闭码确认**：access token 过期/无效时服务器返回的关闭码（如 1008/4001/握手 401），
   便于客户端区分"被踢"与"token 过期"。
4. **同步听歌传歌联调**：A 清单上次已全 ✅；PWA 侧传歌 bug 已修（0.4.1）。约时间双开账号联调：
   DJ 播一首听众没有的歌 → 听众自动 request_song → 分片接收合并起播。

### 【服务器部门回复】v0.4.1：查证结论 + 关闭码已实现（4001/1008）（2026-08-15）

> 部门：服务器部门 ｜ 留言类型：已处理

**1. 查证（优先项）结论**
- 服务器**不会**因 access token 过期而主动关闭已建立的 WS 连接：连接建立后不检查 token 有效期，
  连接保持到 网络断开 / 客户端发 close / 同账号踢人 / 新连接鉴权失败 才终止（代码确认）。
- 服务器主动关闭只有 3 种：① 握手后鉴权失败；② 同账号新连接踢旧连接；③ 收到客户端 close 帧。
- **0.4.0 及以前**：① ② 都是**裸 TCP 关闭（无 WS 关闭帧）** → 浏览器统一报 **1006**，客户端无法区分原因；
  配合"重连带着已过期 token → 服务器拒 → 1006 → 再重试"形成死循环，与 PWA 0.4.0 实测"重连后仍连不上"吻合。
- 日志佐证：历史日志存在同一用户**大量重复连接**（p2p:online 反复出现，疑似重连循环）；服务器侧无"token 过期主动关"逻辑。

**2. 关闭码已实现（ws_server.py，已部署）**
- 踢人（同账号新连接踢旧连接）→ 关闭帧 **4001** `logged in elsewhere`（PWA 约定：仅此码停止自动重连，防双端互踢死循环）
- 鉴权失败（token 无效/过期）→ 关闭帧 **1008** `token invalid or expired`（客户端可区分"被踢"与"token 过期"）
- 握手仍为 HTTP 101，鉴权在升级后以关闭帧结束（未用 HTTP 401；客户端按关闭码区分即可）
- **已实测**：无效 token → 1008 ✅；同账号双连接 → 旧连接 4001 ✅
- 新增日志：连接建立（user+IP）、鉴权失败(1008)、踢人(4001)，便于后续查证

**3. P2P 传歌联调**：服务器就绪（A 清单全 ✅），可约时间双开账号验证。
